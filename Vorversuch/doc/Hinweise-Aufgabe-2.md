...
\end{equation}
$$
gelöst, wobei $A$ die Amplitude, $\omega$ die [Kreisfrequenz](https://de.wikipedia.org/wiki/Kreisfrequenz) und $\phi$ eine freie Phase der Schwingung sind. Die Parameter $A$ und $\phi$ werden durch die Anfangswerte des betrachteten Problems festgelegt; $\omega$ folgt aus einer Sekundärgleicung zu Gleichung **(1)** und ist eine Eigenschaft des Pendels. Mit Hilfe dieses Modells können Sie bei gegebenem $\ell$ die Größe von $g$ mit Hilfe der Gleichung
$$
\begin{equation}
